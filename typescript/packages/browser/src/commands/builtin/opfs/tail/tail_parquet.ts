...
async function tailParquetCommand(
  ...
  try {
    ...
    return [
      ...
      new IOResult({
        exitCode: 1,
        stderr: ENC.encode(`tail: ${first.original}: failed to read as parquet: ${msg}\n`),
      }),
    ]
  }
}

export const OPFS_TAIL_PARQUET = command({
  name: 'tail',
