...
async function grepFeatherCommand(
  ...
  try {
    ...
    return [
      ...
      new IOResult({
        exitCode: 1,
        stderr: ENC.encode(`grep: ${first.original}: failed to read as feather: ${msg}\n`),
      }),
    ]
  }
}

export const OPFS_GREP_FEATHER = command({
  name: 'grep',
