...
async function fileFeatherCommand(
  ...
  try {
    ...
    return [
      ...
      new IOResult({
        exitCode: 1,
        stderr: ENC.encode(`file: ${first.original}: failed to read as feather: ${msg}\n`),
      }),
    ]
  }
}

export const OPFS_FILE_FEATHER = command({
  name: 'file',
