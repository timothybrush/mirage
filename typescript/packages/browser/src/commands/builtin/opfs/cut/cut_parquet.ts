...
async function cutParquetCommand(
  ...
  try {
    ...
    return [
      ...
      new IOResult({
        exitCode: 1,
        stderr: ENC.encode(`cut: ${first.original}: failed to read as parquet: ${msg}\n`),
      }),
    ]
  }
}

export const OPFS_CUT_PARQUET = command({
  name: 'cut',
