...
async function cutFeatherCommand(
  ...
  try {
    ...
    return [
      ...
      new IOResult({
        exitCode: 1,
        stderr: ENC.encode(`cut: ${first.original}: failed to read as feather: ${msg}\n`),
      }),
    ]
  }
}

export const OPFS_CUT_FEATHER = command({
  name: 'cut',
