...
from mirage.core.filetype.parquet import grep as parquet_grep
...
@command("grep", resource="redis", spec=SPECS["grep"], filetype=".parquet")
async def grep_parquet(
    ...
    **_extra: object,
    ...
    try:
        ...
        result = parquet_grep(raw, pattern, ignore_case=i)
        if c:
        ...
        return None, IOResult(
            exit_code=1,
            stderr=f"grep: {paths[0].original}: failed to read as parquet: {e}"
            .encode(),
        )
