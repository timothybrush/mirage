...
from mirage.core.filetype.orc import cut as orc_cut
...
@command("cut", resource="redis", spec=SPECS["cut"], filetype=".orc")
async def cut_orc(
    ...
    **_extra: object,
    ...
    if f is None:
        return None, IOResult(
            exit_code=1,
            stderr=b"cut: -f required for orc files (column names)",
        )
    if c is not None:
        return None, IOResult(
            exit_code=1,
            stderr=b"cut: -c not supported for orc; use -f",
        )
    try:
