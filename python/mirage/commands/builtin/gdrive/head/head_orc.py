...
from mirage.core.filetype.orc import head as orc_head
    ...
    **_extra: object,
    ...
    if c is not None:
        return None, IOResult(
            exit_code=1,
            stderr=b"head: -c not supported for orc files",
        )
    try:
        ...
        return None, IOResult(
            exit_code=1,
            stderr=f"head: {p.original}: failed to read as orc: {e}".encode(),
        )
