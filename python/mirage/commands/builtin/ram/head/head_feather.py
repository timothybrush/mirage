...
from mirage.core.filetype.feather import head as feather_head
...
async def head_feather_provision(
...
@command("head",
         ...
         spec=SPECS["head"],
         filetype=".arrow",
         provision=head_feather_provision)
@command("head",
         resource="ram",
         spec=SPECS["head"],
         filetype=".ipc",
    ...
    **_extra: object,
    ...
    if c is not None:
        return None, IOResult(
            exit_code=1,
            stderr=b"head: -c not supported for feather files",
        )
    try:
        lines = int(n) if n is not None else 10
        raw = await _read_bytes(accessor, paths[0])
        result = feather_head(raw, n=lines)
        return result, IOResult(reads={paths[0].strip_prefix: raw},
                                cache=[paths[0].strip_prefix])
    except Exception as e:
        return None, IOResult(
            exit_code=1,
            stderr=f"head: {paths[0].original}: failed to read as feather: {e}"
            .encode(),
        )
